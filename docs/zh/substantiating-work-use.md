...
| 可另扣 | 设备折旧 | 设备折旧 |
| **记录要求** | **全年每一次在家工作的实际工时**（PCG 2023/1，2023-03-01 起；四周代表期不够） | 每项所报费用的发票与账单，加上工时记录 —— 实际工时，或能体现常规模式的四周代表期日志 |

选固定费率法后不得再单独扣宽带与话费。

**专用工作区域不是实际成本法的一般前提。** 没有专用房间，照样按工作比例分摊各项费用。专用房间解锁的只是一小部分额外项目 —— 主要是该房间的清洁费，按其面积占整套住宅的比例分摊，再按私人使用和其他家庭成员使用的比例调减。占用类费用（租金、房贷利息、市政费）属于另一套且严格得多的判定，受雇人员通常不适用。

实际成本法下，连续四周代表期是明确允许的：工时记录只要能体现常规模式即可采用；逐项计费的话费与网费也可按四周算出工作比例，再套用到全年。
